...
## NSFC Proposal Mode (NSFCAgent)

**SSOT:** `proposal/source/**` (Markdown + YAML). Exports are derived in `proposal/output/**`.

**S-tier adversarial QA (2 rounds max):**
- Panel critics (read-only): `nsfc-reviewer` + `nsfc-evidence-auditor` + `nsfc-consistency-checker` + `nsfc-compliance-guard` + `nsfc-citation-verifier`
- Fixers: `nsfc-designer` (primary) + `nsfc-writer` (decisions + integration)
- Humanizer gate (full coverage, zh-CN + EN): `nsfc-humanizer`

**Hard gates before export:**
- Required sections match `nsfc case.pdf` skeleton
- Strong claims bound in claim registry
- Cross-section consistency passes
- Humanizer gate passes (no AI-pattern writing / no unbound puffery)
