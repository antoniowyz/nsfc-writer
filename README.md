...
## NSFCAgent Quick Start Prompt (New Workflow)

If you want to use the **NSFC proposal workflow** added in this repo (SSOT + multi-agent + adversarial QA + humanizer gate + robust export), start Claude Code in the repo root and paste this:

> I am starting to work on **NSFCAgent** in this repo. My goal is to draft and iteratively improve an **NSFC application** (aligned to the section skeleton in `nsfc case.pdf`) and export a Word document at the end.
>
> Please read and follow the workflow configuration in this repo. For NSFC writing:
> - Treat `proposal/source/**` (Markdown+YAML) as the **single source of truth**. Never hand-edit `proposal/output/**`.
...
>
> When ready to export, run:
> - `python scripts/nsfc/orchestrate.py check --mode strict` (or `--mode draft` early on)
> - `python scripts/nsfc/orchestrate.py export-docx --mode strict --output proposal/output/NSFC_proposal.docx`
>
> Start by asking me for the minimal metadata needed to populate `proposal/source/00_metadata.yaml`, then propose a plan.
